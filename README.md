...
- Long takes are guarded against the model's one bad habit: on sparse stations it can slowly amplify a hiss bed it has fed back to itself through its own audio history. The backend watches the quiet moments of each stream and, when their floor audibly rises and brightens for a sustained stretch, crossfades onto a fresh model state mid-stream - a subtle track change instead of a slowly degrading one
- Raw PCM travels over a WebSocket into one AudioWorklet read cursor. A fixed makeup gain undoes the codec's deliberate headroom, a small slow station trim, a transparent limiter, and an output ceiling make station changes consistent without pumping or lifting the noise floor
- Eight-bit weights, a specialized per-frame step loop, GPU keepalive, short startup chunks, and adaptive 10-12 layer codec output keep generation ahead of playback. The browser starts from a sub-second reservoir instead of hiding deficits behind seconds of buffering
...
The two canvases draw their matrix as **liquid ink**. The cells are dots, as they always were; what is new is what happens between two of them. A dot grows with how much ink the underlying metaball field put there, and once it nears a neighbour of the same shade the two pull a membrane across the gap that leaves each circle along its own tangent and pinches at the waist — so a pair reads as one poured shape rather than as two beads and a bar, and a saturated region floods into a single mass with a gooey edge. The connector is the standard metaball construction ([Hiroyuki Sato](http://shspage.com/aijs/en/#metaball), by way of [kynd's Paper.js port](http://paperjs.org/examples/meta-balls) and [Varun Vachhar's derivation](https://varun.ca/metaballs/)); it lives in `frontend/lib/ink-render.ts` and `frontend/test/ink-render.test.mjs` pins its geometry down, tangency included.

- The **visualiser** is a ring of music struck onto the same square dot grid as everything else on the page. One metaball per frequency band bulges a gooey annulus outward where the music is loud; the lattice underneath never bends - the round wave lights whichever grid cells it reaches and a hole is kept clear for the transport button. The wave is painted as nested liquid bodies - a dim skirt where the field is only breathing on the panel, the mass proper inside it, and a bright one-dot crest riding the surface as honest quantised matrix dots. Cells at the edge swell in from nothing and shrink away to nothing as the surface moves, so the wave's edge bleeds rather than blinking, and the quietest ink is honestly separate beads - joining up is something the music does
- The **cat** is a tamagotchi that watches you work. It blinks on its own, nods on the beat while the music plays, narrows its eyes while the focus timer runs, perks an ear when you add a task, hops with sparkles when you finish one - three hops if that was the last one - and nods off if you leave it alone. The scene is drawn cell by cell every frame in `frontend/lib/pet-scene.ts`, which is also where you would go to redraw it. Its fur pours as one ink mass with the one-cell rim reading as a liquid outline around it; the unlit panel and the sockets cut out of the fur stay plain unfused dots, or the background would run together into a sheet
- The **background** is deliberately almost nothing, and it costs almost nothing: a few large ASCII marks in the ENDLESS geometric sans, drifting on CSS keyframes and leaning towards the cursor, with the cursor itself dragging **liquid ink** across the page's dot matrix — droplets laid along its path that swell, fuse through the same metaball membranes as everything else, and evaporate. The ambient layer runs entirely on the compositor (the cursor arrives as two custom properties and everything downstream is a composited CSS `translate`), and the ink samples only the lattice cells inside its own trail's bounding box, in a draw loop that stops the moment the last droplet dries — so a still cursor costs nothing at all. ENDLESS is not redistributable here, so drop a woff2 build at `frontend/public/fonts/endless.woff2` ([source](https://www.behance.net/gallery/247864363/ENDLESS-Geometric-Sans-Serif-Free-Font)); until it is there the marks fall back to the monospace stack and nothing breaks
...
- Everything honours `prefers-reduced-motion`: the animation loops never start, the background neither drifts nor follows the cursor, and both canvases draw one resting frame instead
...
| `MRT_FAST_ENGINE` | `1` | Specialized streaming step: per-codebook `to_logits` slicing, cached conditioning encoding, hoisted per-frame constants, and clean RVQ truncation. Set `0` for the stock step |
